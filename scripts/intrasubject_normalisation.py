...
hemis=['lh','rh']
demo=nb.load(subject_dir+subject_ids[0]+'/surf_meld/'+hemis[0]+measures[0])

for h in hemis:
    for s in subject_ids:
        demo=nb.load(subject_dir+s+'/surf_meld/'+h+measures[0])
        cortex=nb.freesurfer.io.read_label(subject_dir + s + '/label/'+h+'.cortex.label')
        for m in measures:
            if not os.path.isfile(subject_dir + s + '/surf_meld/'+h+'.intra_z'+m):
                subject_measure=io.load_mgh(subject_dir+s+'/surf_meld/'+h+m)
                z_measure=(subject_measure-np.mean(subject_measure[cortex]))/np.std(subject_measure[cortex])
                io.save_mgh(subject_dir+s+'/surf_meld/'+h+'.intra_z'+m,z_measure,demo)
        #only do FLAIR measures if present
        if os.path.isfile(subject_dir + s + '/surf_meld/'+h+flair_measures[0]):
